...
## 주요 기능
- **간트 타임라인**: 팀 → 메인태스크(그룹 헤더) → 서브태스크(레인 압축 막대). 일/주/월 줌, 오늘 기준 자동 스크롤, 헤더·좌측 라벨·팀 헤더 sticky 고정.
- **메인태스크** = 막대 없는 묶음. 일정은 서브태스크 범위로 자동 계산. 진행률 = 서브 progress 가중평균. 좌측 컬러 바 색 지정(`color`), 라벨 드래그로 순서 변경.
- **서브태스크** = 겹치지 않으면 같은 레인에 패킹. 막대 안에 제목·기간·% 표시, 완료는 검정 해치, 고유색(15색 팔레트).
- **마일스톤**: 막대 위 삼각형(▼)+텍스트. 우클릭 추가 / 드래그로 날짜 이동 / 편집창 목록 관리. **완료 토글**(편집창 체크·우클릭·마커 클릭), 기한 지난 미완료는 빨간 ⚠ 경고.
- **편집 패널**: 서브태스크 카드형(접기·펼치기, 드래그 순서변경), 색 팝업, 진행률 슬라이더, 삭제 확인.
- **"서브태스크 N개 ▼"** 호버 시 칩 팝업 → 클릭하면 해당 시작점으로 스크롤. 담당자 알약 표시.
